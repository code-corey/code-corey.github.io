...
tag:
  ...
  - NoExecute
  - 苏格拉底对话
description: 第 10 课挂过谢客牌、写过关羽书，这一课把牌与书的规则打穿：Exists 不写 value 行不行、节点挂两块牌要几张书、NoExecute 挂牌瞬间全场清退（有管理人的自动回血、裸 pod 死透、没入座的毫发无损）、每个 pod 出厂自带「忍坏节点 300 秒」的两张书（学生自己翻 spec 挖到的）、空 key 万能书、控制节点出厂牌——顺手给第 10 课那笔没开成奖的 3/3/0 押注补了开奖。中间照例有翻车实录：describe 折行漏看、老师两次预测被真实输出打脸。
---

> **CKA 通过之路 · 第 12/12 篇**
> 上一篇：[《节点也有标签——调度开篇》](/云原生/cka/cka-10-scheduling) · 下一篇：[《数据的座位——PV、PVC 与 StorageClass》](/云原生/cka/cka-12-storage)

---

## 写在前面

cka-10 结尾挂了一块 `gpu=true:NoSchedule` 的谢客牌，写了张 `Equal` 容忍书，还留了一问没答：**`Exists` 不写 value 的书，到底行不行？** 这一课就从这道悬案开庭，把牌与书的规则全部打穿。
...
> ① 悬案：Exists 无 value → ② 两块牌，一张书不够 → ③ NoExecute 挂牌清场 → ④ 摘牌回血与备份复活 → ⑤ 出厂自带的 300 秒书 → ⑥ 万能书与控制节点出厂牌 → ⑦ 结课出题

---

## 第 1 课：悬案开庭

先押注再开奖，选项就两个：A 无效（没写 value 等于白纸）；B 有效（比 Equal 更宽，只要键叫 gpu，值随便）。学生押 A。挂牌、递书：
...
A 被驳回，5 秒入座。B 的后半句「值随便认」再用一次换值实验钉死——摘牌、挂一块**同键不同值**的牌（true→false），同时递两张新申请：

```bash
kubectl taint node demo-control-plane gpu=true:NoSchedule-
kubectl taint node demo-control-plane gpu=false:NoSchedule
kubectl apply -f ~/tol-equal.yaml -f ~/tol-exists2.yaml   # Equal 写死 "true"；Exists 不写 value
...
两种书的性格一张表看清：**Equal 较真**（键对值不对，翻脸 Pending）、**Exists 洒脱**（只认键，值换了照坐）。官方匹配规则一句话：键相同、效果相同，且满足其一——Exists（不比 value）或 Equal（value 也相等）。`operator` 这个空格不填的话，默认 Equal。

顺带补上第 10 课欠的词汇账：`operator` 不是命令，是容忍书这张小表上的一个空格（空格名: 填的内容），问的是「怎么比」。

## 第 2 课：两块牌，一张书不够

挂第二块牌 `tier=mgmt:NoSchedule`，验牌时出了第一次事故：
...
预告两块、只见一块。停下查证，用 jsonpath 直读节点档案的污点数组：
...
两块都在——是 describe **放不下会折行**，续行没有 Taints 字头，被 `grep "^Taints"` 吃了：

```bash
kubectl describe node demo-control-plane | grep -A 1 "^Taints"
```

```text
Taints:             gpu=false:NoSchedule
                    tier=mgmt:NoSchedule
```

教训：查污点最可靠的是 jsonpath。然后是本课最重的事故：我在讲义里贴了 delete/apply 的「输出」，实际没跑——下一步真实输出里 AGE 显示 15m（15 分钟前的老 pod，根本没删过），当场拆穿。当堂自首，规矩改成**先跑后写**。真跑：
...
学生自己写出了救活方案——tolerations 列表再加一项：

> **🧑‍🎓 学生：** `- key: "tier" operator: "Exists" effect: "NoSchedule"` 再加一项这个就行了

照抄执行（删了重建，容忍书锁死不许原地改），4 秒 Running。规则钉死：**节点上每块 NoSchedule 牌都得有对应的容忍书消化，一块都不能欠。**

## 第 3 课：NoExecute 挂牌清场

到目前为止的牌都只挡新客。换成 NoExecute 呢？学生押注原话只有一句：

> **🧑‍🎓 学生：** 直接会被赶出来，其他人也会被赶出来

动手前先做两件负责任的事。普查谁有管理人（`ownerReferences` = 担保人，ReplicaSet = Deployment 派的小组长，pod 死了会补；`<none>` = 裸 pod，死了没人补）：
...
一张表三种命运，学生的预测两句全中：没带书的**全被赶**（不分新旧客）；管理人**立刻开始补位**；而 `tol-equal`——Pending 了 28 分钟、从没入过座的那个——毫发无损，**Pending 是防空洞**，驱逐是节点干的事，它不在任何节点上。

## 第 4 课：摘牌回血与备份复活

摘 NoExecute 后我出了个题（谁会自己回来），学生押 B：有管理人的自动回来，裸的死透。开奖时我先自首了一个出题坑——我说「牌一摘就能入座」，错了：
...
原因：gpu/tier 两块 NoSchedule 还挂着。摘光：
...
B 验证完毕。轮到救裸 pod——我笃定「148 行的备份档案混着运行时字段，直接 apply 会报错」，学生跟着押了 B。真实输出：
...
**老师的预测被真实输出打脸**——apiserver 创建对象时会自己忽略/重置 status 类字段，不拿它拒绝你。这堂课师生一起挨真实输出的打，规矩面前平等。

## 第 5 课：每个 pod 出厂自带的两张书

课中学生在 app1 的档案里自己翻到了一个东西，发问：
...
验证：hello 的 pod 身上一模一样——我们从来没给谁写过这书，**是 k8s 在每个 pod 创建时自动塞的**（门卫机制 DefaultTolerationSeconds）。它们防的不是我们随手起的 gpu/tier，而是内置牌：节点不健康（not-ready）、节点失联（unreachable）——**NoExecute 平时最大的用户是 k8s 自己**，它用这块牌处理坏节点。没保住 app1 的原因也清楚了：键不匹配，maintenance ≠ 内置键。

为什么默认忍 300 秒而不是立刻赶？学生答「网络波动，过一会儿就恢复」——对了一半，滤抖动那半。补齐另外两半：立刻赶人 = pod 补去别的节点，拉镜像重建一整套，节点一恢复全是白折腾；但又不能永远等（真死机服务永远缺副本）。300 秒是平衡点。

再亲手玩一次 15 秒迷你版——容忍书写 `tolerationSeconds: 15`：
...
15 秒生死线精确生效。出厂的 300 秒，就是这机制的全员默认版。

## 第 6 课：万能书与控制节点出厂牌

容忍书还有个特殊写法——`key` 留空、只填 `operator: "Exists"`。按官方文档：匹配一切键和值。学生这次学乖了，直接押 B：
...
牌挂着，6 秒入座——**空 key + Exists = 万能书**，什么牌都拦不住。谁用这种书？系统守护进程：网络组件必须在每台节点上跑，不能被任何牌拦。

最后一块拼图：**控制节点出厂牌**。我们的 pod 从第一天起全落在 demo-control-plane 上，生产集群里这不可能——真正的控制节点出厂自带 `node-role.kubernetes.io/control-plane:NoSchedule`（「管理层地盘，普通 pod 别来」），kind 建单节点集群时把它摘了。挂回去试试：
...
普通 pod 立刻进不来。这一幕顺手给第 10 课那笔没开成奖的押注补了开奖：学生当时押 6 副本分布 3/3/0（两个工作节点各三，控制节点零），理由「控制节点管拍板不该干活」——生产多节点里，控制节点出厂带牌，普通 workload 落不上去，**3/3/0 就是标准结局**。押注跨了一课，奖终于开了。
...
内容满分，但藏着**考场翻车坑**：他写的 `operator:"Equal"` 冒号后没空格——YAML 规定冒号后必须跟空格才算键值对，紧贴着写整份合同解析失败，等于零分。
...
- 多块牌 = 滤网：被容忍的划掉，剩下的只要有一块 NoSchedule 就不许入座，一块不能欠
- 三种力度对已入座 pod：NoSchedule 不赶、NoExecute 全赶；没入座的 Pending pod 谁也赶不着
- 挂 NoExecute 的众生相：有管理人的自动回血、裸 pod 死透（要么挂 Deployment 下、要么 YAML 进 git，备份能救）、tolerationSeconds 给宽限期（出厂默认忍坏节点 300 秒）
- 空 key + Exists = 万能书（系统守护进程专用）；控制节点出厂自带 NoSchedule 牌——单节点 kind 被摘掉了，所以我们的 pod 才落得上控制节点
- 本课四次翻车照例入册：describe 折行漏看、编造输出二犯（改先跑后写）、出题忘残留牌、备份预测翻车——真实输出是唯一的裁判
...
➡️ 下一篇：[《数据的座位——PV、PVC 与 StorageClass》](/云原生/cka/cka-12-storage)——pod 是随时可以被赶走的房客，那数据呢？pod 死了数据放哪、谁来管？
