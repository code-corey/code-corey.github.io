...
- **CLI**：命令行工具，在终端里敲命令用的那种「黑窗口」软件，程序员的主力工具形态。
- **会话日志**：你和 AI 干活的完整记录——什么时候用了哪个模型、处理了多少字，都留在本地文件里。
- **Token**：AI 计费的最小文字单位，约等于一个汉字或半个英文单词。AI 的账单本质上是「按字数算的电报费」。
- **订阅 vs API 计费**：前者像包月不限量自助餐，后者像按字数打表。两者口径不一样，想算清楚到底花了多少钱其实很麻烦。
- **成本可见性**：先把「钱花哪了」看清楚，才谈得上省钱——和家里记账一个道理。

### 这篇到底在说什么

很多公司同时用好几个 AI 编程工具，但每个月账单来了只知道总数，不知道哪个项目烧的钱、哪个模型最费。这个开源小工具把你电脑上现成的干活日志读出来，自动算出「哪个项目、哪天、用了哪个模型、花了多少」，相当于给 AI 用度装了个电表。开源 5 天拿了 818 个星，说明「AI 到底帮我花了多少钱」是个普遍的头疼事。

### 这跟普通人有什么关系

个人用户：装上它就能知道自己每天跟 AI 聊天写码到底「烧」了多少额度，月底不再被账单惊吓。团队管理者：终于有数据决定哪些活值得交给 AI、哪些活用 AI 反而更贵。一句话：想控制 AI 开销，第一步是先能看见它。
