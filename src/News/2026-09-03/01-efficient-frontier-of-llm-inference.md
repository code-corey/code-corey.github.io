...
- **推理**：模型训练完之后的干活阶段——你问它答。占 AI 服务成本的绝大部分。
- **延迟**：你发出问题到收到第一个字的时间，相当于餐厅「上第一道菜的速度」。
- **吞吐**：同时服务所有人的总处理能力，相当于「这家店一小时能出多少道菜」。
- **批处理**：把多个用户的请求拼在一起算以提高效率，但拼太多，每个人等得就更久——典型的取舍。
- **效率前沿**：经济学概念——延迟、吞吐、成本三者不可能同时最优，任何配置都是三者之间的一个折中点，好的优化是找到「最划算的那个折中」。

### 这篇到底在说什么

老板总问：「能不能又快、又便宜、又能扛住所有人同时用？」这篇一线 AI 服务商的文章回答：不能，这是物理规律。三个指标像三角形的三条边，拉长一条必然牺牲另一条。真正有价值的做法是先想清楚业务要什么（比如客服机器人要快、离线分析要便宜），再把技术配置精准对到那个需求点上。文章出自真实服务海量用户的厂商，不是纸上谈兵。

### 这跟普通人有什么关系

用 AI 产品时遇到的「卡」「等」「限流」，背后都是这个三角取舍。企业采购 AI 服务时，这篇文章提供了一副「透视镜」：厂商宣传的单一漂亮指标没意义，要问「在我这个场景下，你的取舍点在哪」。自建服务器的团队也能用它自查自己的配置是不是浪费钱。
